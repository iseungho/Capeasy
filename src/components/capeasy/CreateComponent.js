        ...
        if (selectedFile) {
            ...
            postVideo(selectedFile)
                .then((data) => {
                    console.log(data)
                    setIsLoading(false);
                    handleUploadAnotherClick(); // 상태 초기화
                    moveToWait(data); // 다음 페이지로 이동
                })
                .catch(error => {
                    setIsLoading(false);
        ...
        <section className="bg-gray-100 bg-cover min-h-screen flex flex-col items-center justify-center">
            {!selectedFile ? (
                <div
                    ...
                    ${isActive ? 'bg-gray-200' : 'bg-white'} flex items-center justify-center
            ...
            {selectedFile && (
                <div className="flex flex-row gap-4 mt-4">
                    ...
                    <button
                        ...
                        disabled={isLoading}
                    >
                        {isLoading ? <BeatLoader color="#ffffff" size={10} /> : '추억 제작하기'}
                    </button>
                </div>
            )}
            {showModal && error && (
